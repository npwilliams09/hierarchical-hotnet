...
def get_parser():
    ...
    parser.add_argument('-cst', '--defCut', required=True, help='defined cut size')
    return parser

#Nathan's Function
def nearestIndex(ls, num):
    #finds index of closest element in list of integers
    return ls.index(min(ls, key=lambda x: abs(x - num)))
...
def find_cut(observed_heights, observed_sizes, expected_heights, expected_sizes, lower_size_bound, upper_size_bound):
    cutNum = get_parser().parse_args(sys.argv[1:]).defCut
    ...
    my_index = nearestIndex(observed_sizes,cutNum)
    cut_height = observed_heights[my_index]
    cut_ratio = ratios[my_index]
